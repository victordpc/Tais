...
protected:
	...
	*/
	...
	struct TreeNode {
		...
		Link iz;
		Link dr;
		int tam_i;// hijos que tiene el nodo de la izquierda + 1
		size_t altura;
		TreeNode(clave_valor const& e, Link i = nullptr, Link d = nullptr,
			int alt = 1, int taman_i = 0) : cv(e), iz(i), dr(d), altura(alt), tam_i(taman_i) {}
	};
...
public:
	...
	int buscarPos(int pos) {
		return consultar(raiz, pos);
	}
...
protected:
	...
	void inserta(clave_valor const& cv, Link & a) {
		if (a == nullptr) {
			...
			int tam_izq;
			if (a->iz == nullptr) tam_izq = 0;
			else tam_izq = contar_hijos(a->iz); // antes de insertar
			inserta(cv, a->iz);
			int tam_izq_desp = contar_hijos(a->iz); // despues de insertar
			if (tam_izq != tam_izq_desp) a->tam_i++;
			reequilibraDer(a);
		}
	...
	static int contar_hijos(Link k1) {
		if (k1 == nullptr) {
			return 0;
		}
		else {
			return k1->tam_i + contar_hijos(k1->dr);
		}
	}
	...
	int consultar(Link a, int pos) {
		if (a == nullptr) {
			return -1;
		}
		else {
			if (a->tam_i > pos) return consultar(a->iz, pos);
			else if (a->tam_i < pos) return consultar(a->dr, pos - a->tam_i);
